...
fn check_useless_packages() {
    ...
    let mut package_versions = PackageVersions::new(FMRI::parse_raw(&"test".to_owned()).unwrap());
    let mut package = Package::new(FMRI::parse_raw(&"test@1".to_owned()).unwrap(), false, false);
    let mut dependencies = Dependencies::new();
    dependencies.add(Dependency::new(&mut DependTypes::Require(
        FMRI::parse_raw(&"another@1".to_owned()).unwrap(),
    )));
    package.add_runtime_dependencies(dependencies);
    package_versions.add_package(package);
    component.add(package_versions);

    let mut package_versions =
        PackageVersions::new(FMRI::parse_raw(&"another".to_owned()).unwrap());
    package_versions.add_package(Package::new(
        FMRI::parse_raw(&"another@2".to_owned()).unwrap(),
        false,
        false,
    ));
    package_versions.add_package(Package::new(
        FMRI::parse_raw(&"another@3".to_owned()).unwrap(),
        false,
        false,
    ));
    component.add(package_versions);
    ...
    fmri_list.add(FMRI::parse_raw(&"test@1".to_owned()).unwrap());
    assert_eq!(components.check_useless_packages(), fmri_list)
}
